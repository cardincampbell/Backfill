...
from app_v2.models.common import CoverageAttemptStatus, CoverageCaseStatus, OfferStatus, OutboxStatus
from app_v2.models.coverage import CoverageCandidate, CoverageCase, CoverageCaseRun, CoverageContactAttempt, CoverageOffer, OutboxEvent
...
class TwilioSMSDeliveryProvider:
    async def send_coverage_offer(
        ...
        *,
        ...
        callback_url = f"{v2_settings.api_base_url}{v2_settings.api_prefix}/providers/twilio/sms/status"
...
async def _advance_case_after_terminal_offer(
    ...
    *,
    ...
    from app_v2.services.coverage import _create_offer_for_candidate

    coverage_case = await session.get(CoverageCase, offer.coverage_case_id)
    shift_id_raw = offer.offer_metadata.get("shift_id")
    shift = await session.get(Shift, UUID(str(shift_id_raw))) if shift_id_raw else None
    ...
    next_candidate = await session.scalar(
        select(CoverageCandidate)
        .where(
            CoverageCandidate.coverage_case_run_id == offer.coverage_case_run_id,
            ~CoverageCandidate.id.in_(
        ...
        .order_by(CoverageCandidate.rank.asc())
        .limit(1)
    )
    if next_candidate is not None and offer.coverage_case_run_id is not None:
        run = await session.get(CoverageCaseRun, offer.coverage_case_run_id)
        next_offer_ttl_minutes = 15
        operating_mode = "standard_queue"
        premium_cents = 0
        if run is not None:
            next_offer_ttl_minutes = max(1, int(run.run_metadata.get("offer_ttl_minutes", 15)))
            raw_mode = run.run_metadata.get("operating_mode", operating_mode)
            operating_mode = raw_mode.value if hasattr(raw_mode, "value") else str(raw_mode)
            premium_cents = max(0, int(run.run_metadata.get("premium_cents", 0)))
        next_offer = await _create_offer_for_candidate(
            session=session,
            coverage_case_id=coverage_case.id,
            coverage_case_run_id=offer.coverage_case_run_id,
            candidate_record=next_candidate,
            shift=shift,
            phase_no=int(offer.offer_metadata.get("phase_no", 1)),
            channel=offer.channel.value if hasattr(offer.channel, "value") else str(offer.channel),
            available_at=reference_time,
            expires_at=reference_time + timedelta(minutes=next_offer_ttl_minutes),
            operating_mode=operating_mode,
            premium_cents=premium_cents,
        )
        coverage_case.status = CoverageCaseStatus.running
        return str(next_offer.id), None

    coverage_case.status = CoverageCaseStatus.exhausted
    coverage_case.closed_at = reference_time
    return None, str(coverage_case.id)
...
async def expire_due_offers(
    ...
    *,
    ...
    for offer in offers:
        ...
        advanced_offer_id, exhausted_case_id = await _advance_case_after_terminal_offer(
            session,
            offer=offer,
            reference_time=reference_time,
        )
        if advanced_offer_id is not None:
            advanced_offer_ids.append(advanced_offer_id)
...
async def apply_twilio_status_callback(
    ...
    *,
    ...
    return {
        ...
        "advanced_offer_id": advanced_offer_id,
...
async def handle_twilio_inbound_reply(
    ...
    *,
    ...
    from app_v2.services import coverage as coverage_service
