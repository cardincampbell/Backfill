...
from app_v2.models.workforce import Employee, EmployeeLocationClearance, EmployeeRole
...
from app_v2.services import delivery as delivery_service
...
async def respond_to_offer(
    ...
    if action == "accepted":
        if shift.seats_filled >= shift.seats_requested:
            raise ValueError("shift is already fully assigned")
        ...
        if not remaining_pending:
            ...
            if next_candidate is not None and offer.coverage_case_run_id is not None:
                ...
                next_offer = await _create_offer_for_candidate(
                    ...
                    available_at=responded_at,
                    expires_at=responded_at + timedelta(minutes=next_offer_ttl_minutes),
                    operating_mode=operating_mode,
                    premium_cents=premium_cents,
                )
                offer.offer_metadata = {
                    **offer.offer_metadata,
                    "next_offer_id": str(next_offer.id),
                }
                coverage_case.status = CoverageCaseStatus.running
            else:
