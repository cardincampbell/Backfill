...
from app_v2.models.coverage import CoverageCase
...
from app_v2.schemas.coverage import CoverageCaseCreate, CoverageExecutionDispatchRequest
...
from app_v2.services import scheduling as scheduling_service
from app_v2.services.scheduler_adapters import (
    ExternalEmployeeRecord,
    ExternalShiftRecord,
    adapter_for_connection,
    build_connection_secret_hint,
)
from app_v2.services.utils import role_code_from_name
...
def _webhook_path(connection_id: UUID | None) -> str | None:
    if connection_id is None:
        return None
    return f"/api/v2/providers/schedulers/webhook/{connection_id}"
...
def default_dispatch_channel() -> str:
    if (
        v2_settings.retell_api_key
        and v2_settings.retell_from_number
        and (v2_settings.retell_agent_id or v2_settings.retell_agent_id_outbound)
    ):
        return "voice"
    return "sms"
