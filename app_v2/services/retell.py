"""
Thin wrapper around the Retell SDK for V2.
"""
...
def _default_call_agent_id(agent_kind: str = "outbound") -> str:
    default_agent_id = v2_settings.retell_agent_id
    if agent_kind == "inbound":
        return v2_settings.retell_agent_id_inbound or default_agent_id
    if agent_kind == "outbound":
        return v2_settings.retell_agent_id_outbound or default_agent_id
    return default_agent_id


def _default_chat_agent_id(agent_kind: str = "outbound") -> str:
    default_agent_id = v2_settings.retell_chat_agent_id
    if agent_kind == "inbound":
        return v2_settings.retell_chat_agent_id_inbound or default_agent_id
    if agent_kind == "outbound":
        return v2_settings.retell_chat_agent_id_outbound or default_agent_id
    return default_agent_id
...
def create_sms_chat(
    *,
    ...
    response = httpx.post(
        ...
        headers={"Authorization": f"Bearer {v2_settings.retell_api_key}"},
        json=payload,
        timeout=30.0,
    )
