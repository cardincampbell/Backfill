...
from app_v2.models.common import OutboxChannel, OutboxStatus, WebhookDeliveryStatus, WebhookSubscriptionStatus
...
from app_v2.schemas.webhooks import WebhookSubscriptionCreate, WebhookSubscriptionUpdate
...
async def process_outbox_batch(session: AsyncSession, *, limit: int = 20) -> WebhookProcessResult:
    ...
    async with httpx.AsyncClient(timeout=v2_settings.webhook_timeout_seconds) as client:
        for event in events:
            ...
            if delivery.attempt_count >= v2_settings.webhook_max_attempts:
                delivery.next_attempt_at = None
                event.status = OutboxStatus.cancelled
                event.processed_at = reference_time
                event.locked_at = None
