...
from app_v2.schemas.onboarding import OwnerWorkspaceBootstrapRequest
from app_v2.services import onboarding, workspace
...
@pytest.mark.asyncio
async def test_v2_bootstrap_owner_workspace_creates_business_location_membership():
    session = FakeSession()
...
@pytest.mark.asyncio
async def test_v2_workspace_expands_business_membership_to_all_locations():
    session = FakeSession()
    business_id = uuid4()
...
def test_v2_workspace_route_returns_locations(monkeypatch):
    auth_ctx = _make_auth_context()
    business_id = uuid4()
    location_id = uuid4()
    ...
    monkeypatch.setattr("app_v2.api.routes.workspace.workspace_service.list_workspace_locations", fake_list_workspace_locations)
