...
from app.schemas.auth import OTPChallengeVerifyRequest
...
@pytest.mark.asyncio
async def test_create_manager_invite_uses_business_name_in_email_subject(monkeypatch):
    business_id = uuid4()
    location_id = uuid4()
    inviter_id = uuid4()
    ...
    monkeypatch.setattr("app.services.invites.messaging.send_email", fake_send_email)
...
@pytest.mark.asyncio
async def test_verify_invite_acceptance_uses_invited_name_to_complete_onboarding(monkeypatch):
    session = FakeInviteSession()
    challenge_id = uuid4()
    business_id = uuid4()
    location_id = uuid4()
...
@pytest.mark.asyncio
async def test_verify_invite_acceptance_requires_onboarding_when_name_missing(monkeypatch):
    session = FakeInviteSession()
    challenge_id = uuid4()
    business_id = uuid4()
    location_id = uuid4()
...
def test_request_manager_invite_challenge_sets_invite_acceptance_purpose(monkeypatch):
    business_id = uuid4()
    location_id = uuid4()
    invite_id = uuid4()
    ...
    monkeypatch.setattr("app.api.routes.invites.invites.get_invite_preview", fake_get_preview)
    monkeypatch.setattr("app.api.routes.invites.auth_service.request_otp_challenge", fake_request_challenge)

    app.dependency_overrides[get_db_session] = override_db
    try:
        client = TestClient(app)
        response = client.post(
            "/api/manager-invites/token_123/request-challenge",
            json={
                "phone_e164": "+15555550123",
                "manager_name": "Jamie Rivera",
            },
