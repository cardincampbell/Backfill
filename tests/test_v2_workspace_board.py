...
from app_v2.schemas.workspace_board import WorkspaceBoardActionSummaryRead, WorkspaceLocationBoardRead
from app_v2.services import workspace_board
...
def test_v2_location_board_route_returns_snapshot(monkeypatch):
    business_id = uuid4()
    location_id = uuid4()
    ...
    monkeypatch.setattr(
        "app_v2.api.routes.workspace.workspace_board_service.get_location_board",
    ...
    try:
        ...
        response = client.get(
            f"/api/v2/workspace/businesses/{business_id}/locations/{location_id}/board"
        )
...
@pytest.mark.asyncio
async def test_v2_location_board_summarizes_roles_workers_and_actions():
