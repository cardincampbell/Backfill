...
def test_business_read_denies_without_membership():
    ...
    try:
        client = TestClient(app)
        response = client.get(f"/api/businesses/{target_business_id}")
...
def test_create_business_bootstraps_owner_membership(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.businesses.businesses.create_business", fake_create_business)
...
@pytest.mark.asyncio
async def test_request_otp_challenge_records_audit(monkeypatch):
...
def test_request_challenge_route_hides_user_existence_and_rate_limits(monkeypatch):
    session = FakeAuthSession()
    existing_user = User(
        id=uuid4(),
        full_name="Existing User",
...
@pytest.mark.asyncio
async def test_request_sign_in_for_unknown_phone_preserves_public_purpose(monkeypatch):
    session = FakeAuthSession()
...
@pytest.mark.asyncio
async def test_verify_otp_challenge_sign_in_creates_user_and_session_when_missing(monkeypatch):
    session = FakeAuthSession()
...
@pytest.mark.asyncio
async def test_verify_otp_challenge_sign_up_creates_user_and_session(monkeypatch):
    session = FakeAuthSession()
    challenge = OTPChallenge(
        id=uuid4(),
...
def test_require_recent_step_up_enforces_recency():
    ...
    stale_auth_ctx = _make_step_up_auth_context(
        verified_at=datetime.now(timezone.utc) - timedelta(minutes=settings.step_up_ttl_minutes + 1)
    )
...
def test_verify_route_sets_http_only_cookie(monkeypatch):
    challenge = OTPChallenge(
        id=uuid4(),
        user_id=uuid4(),
    ...
    monkeypatch.setattr("app.api.routes.auth.auth.verify_otp_challenge", fake_verify)

    app.dependency_overrides[get_db_session] = override_db
    try:
        client = TestClient(app)
        response = client.post(
            "/api/auth/challenges/verify",
            json={
