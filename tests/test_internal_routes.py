...
def _forecast_backtest_fixture(forecast_run: LaborForecastRun) -> ForecastBacktestRead:
    bucket = forecast_run.points[0]
    return ForecastBacktestRead.model_validate(
        {
            "labor_forecast_run_id": forecast_run.id,
            ...
            "as_of": forecast_run.completed_at,
            "actuals_finalization_policy_version": "bucket_plus_48h_v1",
            "summary": {
                "finalized_bucket_count": 1,
                "pending_bucket_count": 0,
                "finalized_bucket_rate": 1.0,
                "headcount_mae": 0.25,
                "headcount_bias": -0.25,
                "labor_hours_mae": 0.25,
                "labor_hours_bias": -0.25,
                ...
                "confidence_calibration": [
                    {
                        ...
                        "bucket_count": 1,
                        "mean_confidence": 0.84,
                        "within_half_headcount_rate": 1.0,
                        "within_one_headcount_rate": 1.0,
                        "headcount_mae": 0.25,
                    }
                ],
            },
            "buckets": [
                {
                    "labor_forecast_point_id": bucket.id,
                    "location_id": bucket.location_id,
                    "role_id": bucket.role_id,
                    "bucket_start": bucket.window_start,
                    "bucket_end": bucket.window_end,
                    "predicted_headcount": 2.0,
                    "predicted_labor_hours": 12.0,
                    "confidence": 0.84,
                    "actual_headcount": 1.75,
                    "actual_labor_hours": 11.75,
                    "headcount_error": -0.25,
                    "labor_hours_error": -0.25,
                    "actuals_completeness_status": "finalized",
                    "actuals_finalized_at": bucket.window_end,
                    "actuals_finalization_policy_version": "bucket_plus_48h_v1",
                    "pending_reasons": [],
                    "forecast_payload": {"source_week_count": 4},
                    "actuals_payload": {"attendance_fact_count": 2},
                }
            ],
        }
    )
...
def test_labor_forecast_backtest_route_returns_backtest(monkeypatch):
    forecast_run = _labor_forecast_run_fixture()
    backtest = _forecast_backtest_fixture(forecast_run)
    captured: dict[str, object] = {}

    async def fake_backtest(_session, *, labor_forecast_run_id, as_of):
        captured["labor_forecast_run_id"] = labor_forecast_run_id
    ...
    monkeypatch.setattr(
        "app.api.routes.internal.forecast_backtesting.build_forecast_backtest",
        fake_backtest,
    )
    ...
    try:
        with TestClient(app) as client:
            response = client.get(
                f"/api/internal/labor-forecasts/runs/{forecast_run.id}/backtest?as_of=2026-05-01T07:00:00Z",
                headers={"X-Backfill-Worker-Key": "worker_test_key"},
            )
        assert response.status_code == 200
        payload = response.json()
        assert payload["labor_forecast_run_id"] == str(forecast_run.id)
        assert payload["summary"]["finalized_bucket_count"] == 1
        assert payload["buckets"][0]["actuals_completeness_status"] == "finalized"
        assert captured["labor_forecast_run_id"] == forecast_run.id
        assert captured["as_of"] == datetime(2026, 5, 1, 7, 0, tzinfo=timezone.utc)
    finally:
        app.dependency_overrides.clear()
