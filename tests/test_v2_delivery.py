...
from app_v2.models.common import CoverageAttemptStatus, CoverageCaseStatus, CoverageRunStatus, OfferStatus, OutboxStatus
from app_v2.models.business import Location, Role
...
async def test_expire_due_offers_advances_next_candidate_and_updates_reliability():
    ...
    session.execute_queue = [[offer], [attempt], []]
    session.scalar_queue = [attempt, next_candidate]
...
async def test_twilio_sms_provider_builds_callback_and_message(monkeypatch):
    ...
    monkeypatch.setattr("app_v2.services.messaging.send_sms", fake_send_sms)
    ...
    assert captured["status_callback"].endswith("/api/v2/providers/twilio/sms/status")
...
async def test_apply_twilio_status_callback_advances_next_candidate_on_failure():
    ...
    session.get_map[(CoverageCaseRun, run_id)] = run
    session.get_map[(Employee, employee_id)] = employee
    session.scalar_queue = [offer, attempt, next_candidate]
    session.execute_queue = [[attempt], []]
...
async def test_handle_twilio_inbound_reply_accepts_latest_offer(monkeypatch):
    ...
    monkeypatch.setattr("app_v2.services.coverage.respond_to_offer", fake_respond_to_offer)
