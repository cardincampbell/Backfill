...
@pytest.mark.asyncio
async def test_respond_to_offer_decline_dispatches_next_blast_batch():
    business_id = uuid4()
    location_id = uuid4()
    role_id = uuid4()
    shift_id = uuid4()
    ...
    offer = CoverageOffer(
        ...
        coverage_candidate_id=current_candidate_id,
        employee_id=employee_id,
        channel="sms",
        status=OfferStatus.pending,
        idempotency_key="offer-main",
    ...
    standby_candidate = CoverageCandidate(
        id=uuid4(),
        coverage_case_run_id=run_id,
        employee_id=standby_employee_id,
        rank=3,
        score=76.0,
        qualification_status="qualified",
        exclusion_reasons=[],
        scoring_factors={"total": 76.0},
        availability_snapshot={"rule_match": True},
        candidate_metadata={"employee_name": "Standby Person"},
    )
    ...
    run = CoverageCaseRun(
        ...
        run_metadata={
            "dispatch_limit": 2,
            "offer_ttl_minutes": 2,
            "operating_mode": CoverageOperatingMode.blast,
        },
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )
    ...
    session.execute_queue = [
        ...
        [next_candidate, standby_candidate],
    ]
    ...
    assert len(offers) == 2
    assert [item.employee_id for item in offers] == [next_employee_id, standby_employee_id]
    assert all(item.status == OfferStatus.pending for item in offers)
...
@pytest.mark.asyncio
async def test_respond_to_offer_accepts_as_standby_when_shift_is_already_filled():
    business_id = uuid4()
    location_id = uuid4()
    role_id = uuid4()
    shift_id = uuid4()
    ...
    case = CoverageCase(
        ...
        priority=100,
        requires_manager_approval=False,
        case_metadata={"confirmed_employee_id": str(uuid4())},
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )
    offer = CoverageOffer(
    ...
    assert responses
    assert offer.status == OfferStatus.accepted
    assert offer.offer_metadata["accepted_as_standby"] is True
    assert offer.offer_metadata["standby_position"] == 1
    assert shift.seats_filled == 1
    assert case.status == CoverageCaseStatus.filled
