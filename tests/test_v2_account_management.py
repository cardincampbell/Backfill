...
def test_v2_delete_empty_location_route_deletes_location():
    ...
    try:
        response = client.delete(f"/api/v2/businesses/{business_id}/locations/{location_id}")
...
def test_v2_delete_location_with_operational_data_returns_conflict():
    fake_session = FakeAccountSession()
    business_id = uuid4()
    location_id = uuid4()
...
def test_v2_revoke_location_manager_access_route_revokes_membership():
    fake_session = FakeAccountSession()
    business_id = uuid4()
    location_id = uuid4()
    membership_id = uuid4()
    ...
    try:
        response = client.delete(
            f"/api/v2/businesses/{business_id}/locations/{location_id}/manager-access/{membership_id}"
        )
        assert response.status_code == 200
