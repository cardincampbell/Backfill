...
from app_v2.models.coverage import AuditLog, OutboxEvent
...
from app_v2.schemas.webhooks import WebhookSubscriptionCreate
...
def test_v2_create_webhook_route_returns_signing_secret():
    business_id = uuid4()
    ...
    try:
        ...
        response = client.post(
            f"/api/v2/businesses/{business_id}/webhooks",
            json={
...
@pytest.mark.asyncio
async def test_v2_enqueue_audit_event_creates_delivery_and_outbox():
    business_id = uuid4()
    session = FakeWebhookSession()
...
@pytest.mark.asyncio
async def test_v2_process_webhook_outbox_success(monkeypatch):
    business_id = uuid4()
    subscription = WebhookSubscription(
        id=uuid4(),
        business_id=business_id,
    ...
    monkeypatch.setattr("app_v2.services.webhooks.httpx.AsyncClient", FakeAsyncClient)
