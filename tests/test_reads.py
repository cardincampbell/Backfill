...
from tests.test_auth import DummySession, _make_auth_context
...
def test_audit_logs_route_returns_scoped_events(monkeypatch):
    ...
    monkeypatch.setattr("app.api.routes.audit.audit_service.list_logs", fake_list_logs)

    app.dependency_overrides[get_db_session] = _override_db
    app.dependency_overrides[get_auth_context] = override_auth
    try:
        client = TestClient(app)
        response = client.get(f"/api/businesses/{business_id}/audit-logs?limit=25")
...
def test_shift_list_forwards_location_and_window_filters(monkeypatch):
    auth_ctx: AuthContext = _make_auth_context(with_membership=True)
    business_id = auth_ctx.memberships[0].business_id
    captured = {}
    ...
    monkeypatch.setattr("app.api.routes.scheduling.scheduling.list_shifts", fake_list_shifts)
