...
### Phase 1 — 3-Week Parallel Build (Core + All Integrations)

Three tracks running simultaneously. They share the same core engine but have independent workstreams.

**Day 1 (all tracks):** Apply for 7shifts developer/partner access, Deputy API sandbox, AND When I Work developer key. Start toll-free number porting. Sign up for Retell. Set up VS Code, Claude Code, Codex. Initialize the repo.

---

**Track A — Core Engine + Native Lite Dashboard**
*Owner: primary dev focus*

Week 1:
- FastAPI backend with SQLite/Postgres
- Data model: restaurants, workers (with consent fields), shifts, vacancies, cascades, outreach attempts, audit log
- Retell inbound call-out agent (connected to 1-800-BACKFILL via Twilio SIP trunk)
- Caller lookup service
- Consent ledger service (log every consent grant/revoke)
- Basic webhook endpoint for Retell function calls

Week 2:
- Native Lite dashboard: restaurant profile setup, add/edit workers, create shifts, view vacancies
- CSV import for bulk worker upload (name, phone, role, certifications)
- SMS-first Tier 1 cascade engine
- Retell outbound voice agent (shift-fill)
- Retell outbound SMS agent
- Manager notification service (SMS to manager on fill/unfill)

Week 3:
- End-to-end testing on Native Lite path
- Outreach history view on dashboard
- Cascade status view (who was contacted, what they said, outcome)
- CSV export for records
- Audit trail viewer

**Native Lite milestone:** A restaurant with no scheduling software can onboard via dashboard or CSV, their workers can call 1-800-BACKFILL to call out, and the cascade fills the shift — all tracked in the dashboard.

---

**Track B — 7shifts Integration**
*Owner: secondary dev focus (or parallel via Codex)*

Week 1:
- Apply for 7shifts API/partner access (do this DAY ONE — may take days to get credentials)
- Read 7shifts API docs: OAuth flow, endpoints for users/shifts/locations, webhook events
- Build 7shifts adapter scaffolding implementing the base integration interface
- Set up OAuth handshake flow (restaurant connects their 7shifts account to Backfill)

Week 2:
- Roster sync: pull employees, roles, contact info from 7shifts → Backfill workers table
- Schedule sync: pull published shifts → Backfill shifts table
- Webhook listener: 7shifts fires event when shift is unassigned or marked open → Backfill creates vacancy automatically
- Availability sync: pull time-off and availability → exclude unavailable workers from cascade

Week 3:
- Write-back: when Backfill fills a shift, update the assignment in 7shifts
- Sync retry logic and error handling
- End-to-end testing on 7shifts path
- Dashboard shows 7shifts-connected restaurants with sync status

**7shifts milestone:** A 7shifts restaurant connects in one click. When a shift becomes vacant in 7shifts, Backfill detects it and runs the cascade. When filled, the assignment writes back to 7shifts. Manager never leaves their normal tool.

---

**Track C — Deputy, When I Work, and Homebase**
*Owner: can start Week 2 once adapter pattern is proven on 7shifts*

Week 2:
- Build Deputy adapter: OAuth 2.0 auth flow, same pattern as 7shifts
- Deputy has full read+write API with webhooks (Insert, Update, Delete triggers)
- Roster sync: pull employees, roles, contact info → Backfill workers table
- Schedule sync: pull published shifts → Backfill shifts table
- Webhook listeners: shift changes, unassigned shifts → vacancy creation
- Build Homebase adapter: read-only roster and schedule ingestion
- Homebase API key setup flow on dashboard
- Pull employee roster → Backfill workers table (one-way sync)
- Pull published schedules → Backfill shifts table (one-way sync)
- All Homebase fill operations run in Native Lite companion layer

Week 3:
- Deputy write-back: when Backfill fills a shift, update the assignment in Deputy
- Deputy sync retry logic and error handling
- Build When I Work adapter:
  - Token-based developer key auth, service APIs per feature
  - Test read endpoints: roster, shifts, schedules, availability
  - Test write endpoints: shift assignment, status updates
  - If write works → full integration mode (like 7shifts/Deputy)
  - If read-only → companion mode (like Homebase, Native Lite holds fill state)
- Dashboard shows Deputy and When I Work-connected restaurants with sync status
- Dashboard shows Homebase-connected restaurants: "Schedule synced from Homebase · Fill operations managed by Backfill"
- End-to-end testing on Deputy, When I Work, and Homebase paths

**Deputy milestone:** A Deputy restaurant connects via OAuth. When a shift becomes vacant, Backfill detects it and runs the cascade. When filled, the assignment writes back to Deputy. Same gold-standard experience as 7shifts.

**When I Work milestone:** A When I Work restaurant connects via developer key. Backfill syncs roster and schedule. If write is supported, fills write back. If not, Native Lite manages the fill workflow. Either way, the restaurant is covered.

**Homebase milestone:** A Homebase restaurant connects their account. Backfill reads their roster and schedule for context. All fill orchestration lives in Native Lite companion layer.

---

**Phase 1 Integration Summary:**

| Platform | Market Position | API Access | What Backfill Reads | What Backfill Writes | Fill Operations Live In |
|----------|----------------|-----------|--------------------|--------------------|----------------------|
| **7shifts** | Multi-location restaurants | OAuth + webhooks | Roster, shifts, availability | Fill confirmations, shift assignments | 7shifts (source of truth) |
| **Deputy** | Seasonal/hourly restaurants + hospitality | OAuth + webhooks | Roster, shifts, schedules | Fill confirmations, shift assignments | Deputy (source of truth) |
| **When I Work** | Hourly workers, restaurants + hospitality | Token-based developer key | Roster, shifts, schedules | TBD — testing write capability | If writable: When I Work. If read-only: Native Lite companion |
| **Homebase** | Small/single-location | API key (read-only) | Roster, schedules | Nothing (read-only) | Native Lite companion |
| **No scheduler** | Independents, food trucks | N/A | N/A | N/A | Native Lite (full truth) |

**Rule for every restaurant:** "We use your scheduler if we can write to it. If we can't, Backfill holds the fill workflow state so nothing breaks."

**Native Lite is the operational backbone for 2-3 out of 5 scenarios.** 7shifts and Deputy allow full write-back. When I Work likely does too but needs verification. Homebase and no-scheduler restaurants depend on Native Lite.

---

### Phase 2 — Agency Partner Layer

**Build:**
- Agency partner directory (data model + seed data from local restaurant staffing agencies)
- Structured request routing (email + structured SMS to start — NOT a lead dump)
- Each request includes: role, shift date/time, pay, location, certs, urgency, deadline
- Each response tracked: accepted/declined, candidate confirmation, fill confirmation
...
- Agency fill status visible on dashboard alongside internal fill status

**Goal:** When Tier 1 internal coverage fails, Backfill routes to agency partners automatically. Restaurant stays inside one workflow. Manager approves before an external worker shows up.

### Phase 3 — Alumni Network + Behavioral Scoring

**Build:**
- Track which workers filled shifts at which restaurants (restaurants_worked field)
...
### Phase 4+ — Future Considerations (deferred, not deleted)

- Backfill-owned worker pool (if agency partner data reveals opportunity)
- HotSchedules/Fourth integration (enterprise chains, partner-gated API, add when customer makes introduction)
- Additional scheduling platform adapters (Toast, Sling, Restaurant365)
...
|------|-----------|---------------------|
| 1 | Telephony + core engine | You call 1-800-BACKFILL, Retell picks up, agent identifies you, creates vacancy in DB |
| 2 | Native Lite + Tier 1 cascade | Manager adds workers via dashboard/CSV → worker calls out → cascade fills shift via SMS → manager notified |
| 2 | 7shifts roster + schedule sync | 7shifts restaurant connects OAuth → employees and shifts appear in Backfill automatically |
| 3 | 7shifts end-to-end | Shift goes vacant in 7shifts → Backfill detects → cascade runs → fill writes back to 7shifts |
| 3 | Homebase companion mode | Homebase restaurant connects → roster/schedule synced → fill operations run in Native Lite |
| 3 | Full demo-ready | Walk into any restaurant — whether they use 7shifts, Deputy, When I Work, Homebase, or nothing — and get them live |
