...
from app.models.common import SchedulerProvider
...
def webhook_secret_for_connection(connection: SchedulerConnection, provider: SchedulerProvider) -> str:
    ...
    if provider == SchedulerProvider.seven_shifts:
        return settings.sevenshifts_webhook_secret
    if provider == SchedulerProvider.deputy:
        return settings.deputy_webhook_secret
    if provider == SchedulerProvider.when_i_work:
        return settings.wheniwork_webhook_secret
    return ""
...
def adapter_for_connection(connection: SchedulerConnection) -> SchedulerAdapter:
    ...
    if provider == SchedulerProvider.seven_shifts:
        client_id = _credentials_value(connection, "client_id") or settings.sevenshifts_client_id
        client_secret = _credentials_value(connection, "client_secret") or settings.sevenshifts_client_secret
    ...
    if provider == SchedulerProvider.deputy:
        client_id = _credentials_value(connection, "client_id") or settings.deputy_client_id
        client_secret = _credentials_value(connection, "client_secret") or settings.deputy_client_secret
    ...
    if provider == SchedulerProvider.when_i_work:
        api_token = _credentials_value(connection, "api_token") or settings.wheniwork_developer_key
    ...
    if provider == SchedulerProvider.homebase:
        api_key = _credentials_value(connection, "api_key") or settings.homebase_api_key
        if not api_key:
