"""add oregon general labor profile

Revision ID: 20260520_0044
Revises: 20260520_0043
Create Date: 2026-05-20 14:40:00.000000
"""
...
def _definition() -> dict[str, object]:
    return {
        "id": _uuid("profile:us_or_general_nonexempt"),
        ...
        "source_urls": [
            ...
            "https://secure.sos.state.or.us/oard/view.action?ruleNumber=839-020-0050",
        ],
...
def upgrade() -> None:
    ...
    fallback_profile_id = bind.execute(
        sa.select(profile_table.c.id).where(profile_table.c.code == "us_or_general_fallback")
    ).scalar_one_or_none()
    ...
    elif fallback_profile_id is not None:
        profile_id = fallback_profile_id
        op.execute(
            profile_table.update()
            .where(profile_table.c.id == fallback_profile_id)
            .values(
                code=definition["code"],
                jurisdiction_code=definition["jurisdiction_code"],
                display_name=definition["display_name"],
    ...
    if fallback_profile_id is not None and existing_profile_id is not None and fallback_profile_id != existing_profile_id:
        op.execute(
            version_table.delete().where(
                version_table.c.labor_rule_profile_id == fallback_profile_id
            )
        )
        op.execute(
            profile_table.delete().where(profile_table.c.id == fallback_profile_id)
        )

    version_id = _uuid("version:us_or_general_nonexempt:1")
    ...
    if existing_version_id is None:
        op.execute(
            version_table.insert().values(
                ...
                change_summary="Seed Oregon meal and rest break profile",
...
def downgrade() -> None:
    ...
    op.execute(
        version_table.delete().where(
            version_table.c.id == _uuid("version:us_or_general_nonexempt:1")
        )
    )
    op.execute(
        profile_table.delete().where(
            profile_table.c.code == "us_or_general_nonexempt"
        )
    )
