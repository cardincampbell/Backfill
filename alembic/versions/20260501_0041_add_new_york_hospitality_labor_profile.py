"""add new york hospitality labor profile

Revision ID: 20260501_0041
Revises: 20260429_0040
...
def _definition() -> dict[str, object]:
    return {
        "id": _uuid("profile:us_ny_hospitality_nonexempt"),
        ...
        "source_urls": [
            "https://dol.ny.gov/hospitality-industry-wage-order-cr146",
            "https://dol.ny.gov/hospitality-wage-order-faq",
            "https://dol.ny.gov/day-rest-and-meal-periods",
            "https://dol.ny.gov/LS611-doc",
        ],
...
def upgrade() -> None:
    ...
    version_id = _uuid("version:us_ny_hospitality_nonexempt:1")
    ...
    if existing_version_id is None:
        op.execute(
            version_table.insert().values(
                ...
                change_summary="Seed New York hospitality spread-of-hours and rest-day profile",
...
def downgrade() -> None:
    ...
    op.execute(
        version_table.delete().where(
            version_table.c.id == _uuid("version:us_ny_hospitality_nonexempt:1")
        )
    )
    op.execute(
        profile_table.delete().where(
            profile_table.c.code == "us_ny_hospitality_nonexempt"
        )
    )
