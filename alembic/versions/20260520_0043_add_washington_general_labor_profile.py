"""add washington general labor profile

Revision ID: 20260520_0043
Revises: 20260520_0042
...
def _definition() -> dict[str, object]:
    return {
        "id": _uuid("profile:us_wa_general_nonexempt"),
        ...
        "source_urls": [
            ...
            "https://apps.leg.wa.gov/WAc/default.aspx?cite=296-126&full=true",
...
def upgrade() -> None:
    ...
    version_id = _uuid("version:us_wa_general_nonexempt:1")
    ...
    if existing_version_id is None:
        op.execute(
            version_table.insert().values(
                ...
                change_summary="Seed Washington meal and rest break profile",
...
def downgrade() -> None:
    ...
    op.execute(
        version_table.delete().where(
            version_table.c.id == _uuid("version:us_wa_general_nonexempt:1")
        )
    )
    op.execute(
        profile_table.delete().where(
            profile_table.c.code == "us_wa_general_nonexempt"
        )
    )
