...
import { V2CoverageControlPanel } from "@/components/v2-coverage-control-panel";
import { EmptyState } from "@/components/empty-state";
import { V2LocationSettingsPanel } from "@/components/v2-location-settings-panel";
import { V2RosterManagerPanel } from "@/components/v2-roster-manager-panel";
import { V2ShiftManagerPanel } from "@/components/v2-shift-manager-panel";
import {
  getV2LocationBoard,
  getV2LocationSettings,
  type V2Workspace,
  type V2WorkspaceBoard,
  type V2WorkspaceLocation,
} from "@/lib/api/v2-workspace";

type LocationDetailV2Query = Record<string, string | string[] | undefined>;

type RenderV2LocationDetailPageArgs = {
  workspace: V2Workspace;
  location: V2WorkspaceLocation;
  query?: LocationDetailV2Query;
  basePath: string;
};
...
type ShiftCardEntry = V2WorkspaceBoard["shifts"][number];
...
function locationMeta(location: V2WorkspaceLocation): string {
  return [
...
function shiftTone(shift: ShiftCardEntry): string {
  ...
  if (shift.pending_offer_count > 0 || shift.delivered_offer_count > 0) return "coverage";
...
function buildRoleSections(board: V2WorkspaceBoard) {
...
function ScheduleBoard({ board }: { board: V2WorkspaceBoard }) {
  const days = buildBoardDays(board.week_start_date);
  const sections = buildRoleSections(board);

  return (
    <section className="v2-board-shell">
      <div className="v2-board-summary">
        <div className="v2-board-summary-item">
      ...
      <div className="v2-board-scroll">
        <div className="v2-board-grid">
          <div className="v2-board-head v2-board-sticky">Team / role</div>
          {days.map((day) => (
            <div key={day.key} className="v2-board-head">
          ...
          {sections.map((section) => (
            <div key={section.role.role_id} className="v2-board-role-section">
              <div className="v2-board-role-row">
                <div className="v2-board-role-cell v2-board-sticky">
                ...
                <div className="v2-board-role-fill" />
              </div>

              <div className="v2-board-row">
                <div className="v2-board-label v2-board-sticky">
                ...
                {days.map((day) => {
                  ...
                  return (
                    <div key={`${section.role.role_id}-open-${day.key}`} className="v2-board-cell">
              ...
              {section.workers.map((worker) => {
                ...
                return (
                  <div key={`${section.role.role_id}-${worker.employee_id}`} className="v2-board-row">
                    <div className="v2-board-label v2-board-sticky">
                    ...
                    {days.map((day) => {
                      ...
                      return (
                        <div key={`${worker.employee_id}-${day.key}`} className="v2-board-cell">
...
export async function renderV2LocationDetailPage({
  workspace,
  location,
  query = {},
  basePath,
}: RenderV2LocationDetailPageArgs) {
  ...
  const board = await getV2LocationBoard(
  ...
  const settings =
    activeTab === "settings"
      ? await getV2LocationSettings(location.business_id, location.location_id)
      : null;
