...
export function ShiftManagerPanel({
  ...
  function handleSave(shiftId: string) {
    ...
    startTransition(async () => {
      try {
        await updateShift(businessId, shiftId, {
  ...
  function handleDelete(shift: ShiftRow) {
    ...
    startTransition(async () => {
      try {
        await deleteShift(businessId, shift.shift_id);
