...
import {
  enrollV2EmployeeAtLocation,
  type V2WorkspaceBoard,
} from "@/lib/api/v2-workspace";

type RoleRow = V2WorkspaceBoard["roles"][number];
type WorkerRow = V2WorkspaceBoard["workers"][number];

type V2RosterManagerPanelProps = {
  businessId: string;
  locationId: string;
...
export function V2RosterManagerPanel({
  businessId,
  locationId,
  canManageTeam,
  roles,
  workers,
}: V2RosterManagerPanelProps) {
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      try {
        const result = await enrollV2EmployeeAtLocation(businessId, {
          ...
          employee_metadata: { source: "workspace_v2" },
        });
        setFeedback({
          tone: "success",
