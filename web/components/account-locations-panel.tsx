...
function formatAddress(location: WorkspaceLocation): string {
  return [
...
export function AccountLocationsPanel({
  workspace,
}: AccountLocationsPanelProps) {
  ...
  const [membershipsByLocation, setMembershipsByLocation] = useState<
    Record<string, ManagerAccessEntry[]>
  >({});
  ...
  async function handleAddLocation(business: WorkspaceBusiness) {
  ...
  async function loadMemberships(
    ...
    try {
      const memberships = await listLocationManagers(businessId, locationId);
  ...
  function toggleInviteForm(location: WorkspaceLocation) {
    setFeedback(null);
    const nextValue =
  ...
  function handleInvite(location: WorkspaceLocation) {
    ...
    startTransition(async () => {
      try {
        const result = await inviteLocationManager(
    ...
    startTransition(async () => {
      try {
        if (membership.entry_kind === "invite") {
          await revokeLocationManagerInvite(
            location.business_id,
            location.location_id,
            membership.id,
          );
        } else {
          await revokeLocationManager(
            location.business_id,
            location.location_id,
            membership.id,
          );
